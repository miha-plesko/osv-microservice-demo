...
VIRTLET_POD="${1:-}"
PROJECT_DIR=$HOME/git-repos/osv-microservice-demo
...
function fun::compose_and_upload {
  ...
  capstan package compose micro --size=200MB --update --boot ${1} --pull-missing
...
if [[ ${1:-} = "--help" || ${1:-} = "-h" ]]; then
  cat <<EOF >&2
Usage: ./upload-images.sh VIRTLET-POD|IMAGESERVER-POD

VIRTLET-POD is id of Pod where virtlet server runs. Query it with:
$ kubectl get pods -n kube-system | grep virtlet
Example value: virtlet-wcn06
...
EOF
  ...
  echo Usage: ./upload-images.sh VIRTLET|IMAGESERVER-POD
  echo Error: missing VIRTLET-POD argument
  exit 0
fi
